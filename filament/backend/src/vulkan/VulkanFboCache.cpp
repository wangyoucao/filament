...
VkFramebuffer VulkanFboCache::getFramebuffer(FboKey config, uint32_t width,
    ...
    VkImageView attachments[MRT::NUM_TARGETS + 1];
...
VkRenderPass VulkanFboCache::getRenderPass(RenderPassKey config) noexcept {
    ...
    struct { VkImageLayout subpass, initial, final; } colorLayouts[MRT::NUM_TARGETS];
    if (isSwapChain) {
    ...
    VkAttachmentReference colorAttachmentRef[MRT::NUM_TARGETS] = {};
    ...
    VkAttachmentDescription attachments[MRT::NUM_TARGETS + 1] = {};
